  ...
  const field = (label: string, value: string, onChange: (v: string) => void, type = 'text') => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      <input
        ...
        required
        style={{ padding: '0.6rem 0.75rem', borderRadius: 8, border: '1px solid #ddd', fontSize: 14 }}
      />
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f5f3' }}>
      <div style={{ width: 360, background: '#fff', borderRadius: 16, padding: '2rem', boxShadow: '0 2px 16px rgba(0,0,0,0.08)' }}>
        <h1 style={{ margin: '0 0 0.25rem', fontSize: 22, fontWeight: 500 }}>💬 ChatApp</h1>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          {error && (
            <div style={{ padding: '0.5rem 0.75rem', background: '#FCEBEB', color: '#A32D2D', borderRadius: 8, fontSize: 13 }}>
              {error}
            </div>
          )}
        ...
        <p style={{ textAlign: 'center', marginTop: '1.25rem', fontSize: 13, color: '#888' }}>
          {mode === 'login' ? "Don't have an account? " : 'Already have an account? '}
          <button
            onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); clearError() }}
            style={{ background: 'none', border: 'none', color: '#534AB7', cursor: 'pointer', fontWeight: 500, fontSize: 13 }}
          >
